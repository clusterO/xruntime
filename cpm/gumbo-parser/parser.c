    ...
    const GumboNode *node, bool is_last)
{
  ...
  case GUMBO_TAG_TD:
  case GUMBO_TAG_TH:
    return is_last ? GUMBO_INSERTION_MODE_IN_BODY : GUMBO_INSERTION_MODE_IN_CELL;
  case GUMBO_TAG_TR:
...
static bool is_html_integration_point(const GumboNode *node)
{
  return (node_tag_in(node, GUMBO_TAG_FOREIGNOBJECT, GUMBO_TAG_DESC,
         ...
         (node_tag_is(node, GUMBO_TAG_ANNOTATION_XML) && (attribute_matches(&node->v.element.attributes,
                                                                            "encoding", "text/html") ||
  ...
  for (int i = open_elements->length; --i > 1;)
  {
    ...
    if (node_tag_is(table_element, GUMBO_TAG_TABLE))
    {
      ...
      gumbo_debug("Found enclosing table (%x) at %d; parent=%s, index=%d.\n",
                  table_element, i, gumbo_normalized_tagname(foster_parent_element->v.element.tag),
...
static void maybe_flush_text_node_buffer(GumboParser *parser)
{
  ...
  {
    append_node(
        parser, parser->_output->root ? get_current_node(parser) : parser->_output->document, text_node);
  }
    ...
    GumboToken *current_token, GumboElement *element)
{
  ...
  element->original_end_tag =
      current_token->type == GUMBO_TOKEN_END_TAG ? current_token->original_text : kGumboEmptyString;
}
                           ...
                           bool is_reconstructing_formatting_elements)
{
  ...
  append_node(
      parser, parser->_output->root ? get_current_node(parser) : parser->_output->document, node);
  gumbo_vector_add(parser, (void *)node, &state->_open_elements);
}
    ...
    GumboParser *parser, GumboToken *token, bool is_li)
{
  ...
  {
    const GumboNode *node = state->_open_elements.data[i];
    bool is_list_tag = is_li ? node_tag_is(node, GUMBO_TAG_LI) : node_tag_in(node, GUMBO_TAG_DD, GUMBO_TAG_DT, GUMBO_TAG_LAST);
    if (is_list_tag)
    {
    ...
    GumboParser *parser, GumboToken *token, GumboTag closing_tag)
{
  ...
  for (int i = 0; i < 8; ++i)
  {
    ...
    int insert_at = gumbo_vector_index_of(
                        &state->_open_elements, furthest_block) +
                    1;
    assert(insert_at >= 0);
...
static bool handle_token(GumboParser *parser, GumboToken *token)
{
  ...
  if (!current_node ||
      ...
      (is_html_integration_point(current_node) && (token->type == GUMBO_TOKEN_START_TAG ||
                                                   token->type == GUMBO_TOKEN_CHARACTER ||
                                                   token->type == GUMBO_TOKEN_NULL ||
                                                   token->type == GUMBO_TOKEN_WHITESPACE)) ||
